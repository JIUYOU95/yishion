...
<ul class="breadcrumb row navigation">
    ...
    <li><a href="#">巡店系统</a></li>
    <li><a href="<?php echo U('score');?>">评分配置</a></li>
    <li class="active">评分方案</li>
    <span class="btn btn-primary rightbut" ng-click="add('<?php echo ($pid); ?>')">新增项目</span>
</ul>
<!--评分配置-->
<div class="row">
    <div class="col-sm-12 plugin">
        <form action="<?php echo U('item_order');?>" method="post">
            <ul class="list-group">
                ...
                <li class="list-group-item" ng-repeat="v in data">
                    <div>
                        ...
                        <span>{{v.name}}</span>
                        <span>
                            <a href="#" ng-click="add_sub(v.id,'<?php echo ($pid); ?>')">新增子项目</a> |
                    ...
                    <ul class="list-group" ng-show="v.reid">
                        <li class="list-group-item" ng-repeat="vo in v.child">
                            <div>
                                ...
                                <span>
                                    <a href="#" ng-click="add_sub(vo.id,'<?php echo ($pid); ?>')">新增子项目</a> |
                            ...
                            <ul>
                                <li class="list-group-item" ng-repeat="vv in vo.child">
                                    <div>
                                        ...
                                        <span>
                                            <a href="#" ng-click="add_sub(vv.id,'<?php echo ($pid); ?>')">新增子项目</a> |
...
<script>
    ...
    function controller($scope, $http, $filter) {
        //项目列表
        $http.get("<?php echo U('get_item');?>",{params:{id:'<?php echo ($pid); ?>'}}).success(function (response) {
            $scope.data=response;
        });
        //项目新增
        ...
        $scope.top_del=function (v,id,title) {
            if(confirm('确定删除！')){
                $http.get("<?php echo U('delete_item');?>",{params:{'id':id,'name':title}}).success(function (data) {
                    alert(data);
                    if(data!='请先删除子项目'){
